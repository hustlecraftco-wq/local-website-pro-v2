...
export default function EmergencyPlumberPage() {
  ...
  // Optimized Scroll Handler (Passive listener for performance)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 selection:bg-cyan-500/30">
      ...
      {/* 1. GLOBAL STYLES FOR ANIMATIONS (Inline for single-file portability) */}
      <style jsx global>{`
        @keyframes fadeInUp {
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'py-4 bg-slate-950/80 backdrop-blur-md border-b border-slate-800' : 'py-6 bg-transparent'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <a href="tel:555-0123" className="flex items-center gap-2 bg-rose-600 hover:bg-rose-500 text-white px-5 py-2.5 rounded-full font-bold text-sm transition-all shadow-lg shadow-rose-900/20 animate-pulse-glow">
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700 text-cyan-400 text-sm font-medium mb-8 opacity-0 animate-fade-in-up">
          ...
          <h1 className="text-5xl lg:text-7xl font-bold tracking-tight mb-8 bg-gradient-to-b from-white to-slate-400 bg-clip-text text-transparent opacity-0 animate-fade-in-up delay-100">
            Emergency HVAC & <br /> Plumbing Experts.
          </h1>

          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed opacity-0 animate-fade-in-up delay-200">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center opacity-0 animate-fade-in-up delay-300">
            <button className="px-8 py-4 bg-white text-slate-950 rounded-full font-bold hover:bg-cyan-50 transition-colors flex items-center justify-center gap-2">
              Book Now
              <Icon name="check" className="w-4 h-4" />
            </button>
            <button className="px-8 py-4 bg-slate-800 text-white border border-slate-700 rounded-full font-bold hover:bg-slate-700 transition-colors">
              View Service Map
            </button>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="max-w-xl">
              ...
              <p className="text-slate-400">We handle everything from minor residential leaks to complex commercial HVAC installations.</p>
            </div>
            <Link href="#" className="text-cyan-400 font-bold hover:text-cyan-300 flex items-center gap-2">
      ...
      <section className="py-24 border-t border-slate-800 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative text-center">
          <h2 className="text-4xl font-bold mb-8">Need help right now?</h2>
          <div className="flex flex-col items-center gap-6">
            <a href="tel:555-0123" className="text-5xl md:text-7xl font-black text-cyan-400 hover:text-cyan-300 transition-colors tracking-tight">
              (555) 012-3456
            </a>
            <p className="text-slate-500">
              Live dispatchers are standing by. <br />
              <span className="text-emerald-400">● Currently typical response time: 12 mins</span>
            </p>
          </div>
        </div>
      </section>
